...
# Streamlit setup
st.set_page_config(page_title="YouTube Discovery Dashboard", layout="wide")
st.title("🎥 YouTube Discovery Dashboard")
...
# Tabs for dashboard
tab1, tab2 = st.tabs(["📈 Viral Topics", "🔍 Find Similar Channels"])
...
with tab2:
    st.header("🔍 Find Similar Channels")
    channel_name = st.text_input("Enter a Channel Name")
    max_subs = st.slider("Max Subscribers", 1000, 10_000_000, value=1_000_000, step=1000)
    min_monthly_views = st.slider("Min Estimated Monthly Views", 10_000, 50_000_000, value=500_000, step=10000)

    if st.button("Find Similar"):
        with st.spinner("Searching for channel and similar creators…"):
            search_params = {
            ...
            target_desc = ch_data["items"][0]["snippet"].get("description", "")
            target_title = ch_data["items"][0]["snippet"]["title"]

            df = get_results(keywords, API_KEY, days)
            df = df[df.Subscribers <= max_subs]
            df["EstimatedMonthlyViews"] = df["Views"] / days * 30
            df = df[df["EstimatedMonthlyViews"] >= min_monthly_views]

            descs = df["ChannelDesc"].fillna("").replace("", "No description available").tolist()
            descs.insert(0, target_desc)
            vectorizer = TfidfVectorizer(stop_words="english")
            tf
